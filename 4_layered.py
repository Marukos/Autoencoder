...
class Encoder(nn.Module):
    ...
    def __init__(self, encoded_space_dim, fc2_input_dim):
        ...
        self.encoder_lin = nn.Sequential(
            nn.Linear(3*3*64, fc2_input_dim),
            nn.ReLU(True),
...
class Decoder(nn.Module):
    ...
    def __init__(self, encoded_space_dim, fc2_input_dim):
        ...
        self.decoder_lin = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Linear(fc2_input_dim, 64*3*3),
            nn.ReLU(True)
        )
        ...
        self.decoder_conv = nn.Sequential(
            nn.ConvTranspose2d(64, 32, 3, stride=2, padding=2, output_padding=0),
            nn.BatchNorm2d(32),
...
def pca_loss(f_device, loss_fn):
    ...
    print('PCA components:', encoded_data.shape[1])
    return dataset_loss.data
...
if __name__ == "__main__":
    ...
    encoder = Encoder(encoded_space_dim=d, fc2_input_dim=64*4)
    decoder = Decoder(encoded_space_dim=d, fc2_input_dim=64*4)
    params_to_optimize = [
    ...
    num_epochs = 50
    array_epoch = []
    array_info = numpy.zeros((num_epochs, 4))

    pca_loss = pca_loss(device, loss_function)
    ...
    for epoch in range(num_epochs):
        ...
        if test_loss <= pca_loss:
            print('AE Test loss was smaller than PCA loss by: %f' % (pca_loss - test_loss))
        else:
            print('PCA loss was smaller than AE test loss by: %f' % (test_loss - pca_loss))
        if epoch % 10 == 0:
    ...
    df.to_excel('4-layered_AE.xlsx', sheet_name='sheet_one')
